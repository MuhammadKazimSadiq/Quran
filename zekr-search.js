...
function indexOfIgnoreDiacritic(src, key) {
  ...
  if (k == key.length) {
    // fully matched
    spaceBefore =
      key.charAt(0) != " " ? src.substring(0, start).lastIndexOf(" ") : start;
...
function highlightWordInText(aWord, textNode, matchDiac, matchCase) {
  ...
  var myIndexOf;
  if (matchDiac) myIndexOf = indexOfMatchDiacritic;
  else myIndexOf = indexOfIgnoreDiacritic;
...
Finder.prototype.find = function (str) {
  ...
  highlightWordInNode(
    str,
    document.getElementById("searchableSection"),
    this.matchDiac,
    this.matchCase
  );
};
...
CurrentPageSearchResult = function () {
  ...
  function focus() {
    ...
    $(item)
      .ScrollTo(500, "original", bh > h ? bh / 5 : 0)
      .attr("className", "jsHighlightFocused");
  }
};
...
function searchResultRecite(but, intrnl) {
  ...
  if (
    !intrnl &&
    lastPlayingSearchItem &&
    b.attr("id") != $(lastPlayingSearchItem).attr("id") &&
    $(lastPlayingSearchItem).attr("playing")
  ) {
