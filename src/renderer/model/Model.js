...
export default class Model {
    ...
    static get(id, joins = []) {
        return new Promise((resolve, reject) => {
            const { selectString, joinString, conditionString, orderString } = this.buildQueryString({joins, conditions: [['id', id]], order: []})
    ...
    static find(joins = [], conditions = []) {
        return new Promise((resolve, reject) => {
            const { selectString, joinString, conditionString, orderString } = this.buildQueryString({joins, conditions, order: []})
            const querystring = `SELECT ${selectString} from ${this.table} ${joinString} ${conditionString} ${orderString}`
            Database.query(querystring)
                .then(res => resolve(res))
                .catch(err => reject(err))
    ...
    static async insert(data) {
        return new Promise((resolve, reject) => {
            ...
            Database.query(querystring)
                .then(() => {
                    Database.query(`SELECT max(id) as id
                    from ${this.table}`)
                        .then(id => resolve(id))
                })
                .catch(err => reject(err))
    ...
    static async update(conditions, values) {
        return new Promise((resolve, reject) => {
            ...
            const valueString = values.map(value => ` ${value[0]} = '${value[1]}' `).join(' , ')
    ...
    static formSelectString(joins) {
        ...
        if (joins.length) {
            ...
            joins.forEach(table => table.cols.forEach(col => joinArray.push(`${table.to_table}.${col[0]} as ${col[1]}`)))
    ...
    static formJoinString(joins) {
        joins = joins.length ? [...joins, ...this.joins] : this.joins
        return joins.length
            ? joins.map(join => ` left join ${join.to_table} on ${join.from_table ? join.from_table : this.table}.${join.from_col} = ${join.to_table}.${join.to_col} `).join(' ')
            : ''
    }
    ...
    static formConditionString(conditions) {
        ...
        const conditionString = conditions.map((condition) => {
            ...
            if(condition.length === 2) return `${this.table}.${condition[0]} = '${condition[1]}' `
            // check if condition is `LIKE` clause
            if(condition[1] === 'LIKE') return `${condition[0]} LIKE '%${condition[2]}%'`
