  ...
  <Combobox v-model="selectedTopics" @update:modelValue="onUpdate()" multiple>
    <div class="relative mt-1">
      ...
      <div
        ...
        <div class="mx-2">
          <ComboboxButton class="flex items-center">
            <PlusCircleIcon class="w-5 text-gray-700/70 dark:text-gray-200/70" />
          </ComboboxButton>
        </div>
        ...
        <div class="my-2">
          <div class="flex flex-wrap items-center gap-2">
            <div v-for="(topic, i) in selectedTopics"
              ...
              <XMarkIcon @click.stop="selectedTopics.splice(i, 1) && onUpdate()"
        ...
        <ComboboxInput
          ...
          :placeholder="selectedTopics.length ? '' : $t('topics_placeholder')" @keydown="onKeyPress($event)"
          @change="query = $event.target.value" />
        <!-- input -->

        <!-- save & cancel icons -->
        <div v-if="showSaveIcon" class="flex gap-2">
          <ArrowDownTrayIcon @click="updateTopics()"
            class="h-4 w-4 cursor-pointer text-gray-600/60 dark:text-gray-200/70" />
          <XMarkIcon @click="cancel()" class="h-4 w-4 cursor-pointer text-gray-600/60 dark:text-gray-200/70" />
        </div>
      ...
      <ComboboxOptions
        ...
        <div v-if="filteredTopics.length === 0 && query !== ''"
        ...
        <ComboboxOption v-if="queryTopic" :value="queryTopic" as="template">
          <div class="relative cursor-pointer select-none bg-gray-100 py-2 pl-10 dark:bg-gray-900 dark:text-gray-100">
        ...
        <ComboboxOption v-for="topic in filteredTopics" as="template" :key="topic.topic_id" :value="topic"
